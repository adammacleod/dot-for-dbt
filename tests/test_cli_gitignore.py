...
def run_cli_in_dir(tmp_path, gitignore_content=None, expect_exit_code=0, input_response=None):
    ...
    try:
        ...
        if input_response is not None:
            with patch("builtins.input", lambda _: input_response), patch("sys.stdout", stdout), patch("sys.stderr", stderr):
                with patch("dot.dot.dbt_command", return_value=["echo", "mocked"]), patch("subprocess.run") as mock_run:
